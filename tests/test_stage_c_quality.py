...
class StageCFinalProvenanceRegressionTest(unittest.TestCase):
    def test_final_receipt_preserves_each_replica_model_artifacts(self):
        ...
        r2.write_text(json.dumps({**base,'replica_id':'R2','training_evidence':{'seed':202,'arm_a_config':{'seed':202},'arm_b_config':{'seed':202}},'model_artifacts':{
            'arm_a_early_cact_sha256':'4'*64,
            'arm_a_final_cact_sha256':'5'*64,
            'arm_b_early_cact_sha256':'6'*64,
            'arm_b_final_cact_sha256':'7'*64,
        }}))
        ...
        self.assertEqual(final['replicas']['R2']['model_artifacts']['arm_b_final_cact_sha256'],'7'*64)
...
class StageCReplicaIndependenceRegressionTest(unittest.TestCase):
    ...
    def test_final_rejects_duplicate_model_artifacts_across_replicas(self):
        import json, pathlib, subprocess, tempfile
        root=pathlib.Path(__file__).resolve().parents[1]
        td=pathlib.Path(tempfile.mkdtemp(prefix="stage-c-duplicate-artifacts-"))
        artifacts={
        ...
        r1=td/"r1.json"; r2=td/"r2.json"; out=td/"final.json"
        r1.write_text(json.dumps({**base,"replica_id":"R1","training_evidence":{"seed":101,"arm_a_config":{"seed":101},"arm_b_config":{"seed":101}}}))
        r2.write_text(json.dumps({**base,"replica_id":"R2","training_evidence":{"seed":202,"arm_a_config":{"seed":202},"arm_b_config":{"seed":202}}}))
        x=subprocess.run(["python3",str(root/"scripts/stage_c_quality_receipt.py"),"final","--r1",str(r1),"--r2",str(r2),"--output",str(out)],text=True,capture_output=True)
        self.assertNotEqual(x.returncode,0)
